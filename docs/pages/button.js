...
function Icon({name}) {
  return View({
    tag: 'span', 
    onMount($el, props) {
      fetch(`https://unpkg.com/@tabler/icons@2.19.0/icons/${props}.svg`).then(res => res.text()).then(svg => {
...
export default function () {
  return DocPage({name: 'Button'}, [
    ...
    Section({title: "With Icon", description: 'You can use tabler-icons.io to get icon that you want'}, [
      ButtonGroup({}, [

      Button({ color: 'primary'}, [ Icon({name: 'home'}), 'Left']),
      Button({ color: 'primary'}, [ 'Right', Icon({name: 'home'})]),
      Button({ color: 'primary'}, [ Icon({name: 'home'}), 'Both', Icon({name: 'check'})]),
      Button({ color: 'primary'}, [ Icon({name: 'home'}), Icon({name: 'check'})]),
    ...
    Section({title: 'Sizes', description: 'Button component supports these sizes'}, [
      Button({ color: 'primary', size: 'sm'}, [ Icon({name: 'check'}), 'Small (sm)']),
      Button({ color: 'primary', size: 'md'}, [ Icon({name: 'check'}), 'Medium (md)']),
      Button({ color: 'primary', size: 'lg'}, [ Icon({name: 'check'}), 'Large (lg)']),
      Button({ color: 'primary', size: 'xl'}, [ Icon({name: 'check'}), 'Extra Large (xl)']),
    ]),
    Section({title: 'Colors', description: 'Button component supports these colors'}, [
      ButtonGroup({}, [
